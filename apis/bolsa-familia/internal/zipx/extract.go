...
func ExtractCSVFiles(zipPath, outDir string) (CSVFiles, error) {
	...
	for _, f := range r.File {
		...
		if detectRole(base) == "bolsa_familia" {
			files.BolsaFamilia = dstPath
		}
	}
...
func DetectInDirectory(dir string) (CSVFiles, error) {
	...
	for _, e := range entries {
		...
		if detectRole(name) == "bolsa_familia" {
			files.BolsaFamilia = full
		}
	}
	if err := validateDetected(files); err != nil {
		return CSVFiles{}, err
...
func detectRole(fileName string) string {
	...
	if strings.Contains(n, "novobolsafamilia") || strings.Contains(n, "bolsafamilia") {
...
func validateDetected(files CSVFiles) error {
	if files.BolsaFamilia == "" {
		return errors.New("CSV obrigatorio nao encontrado: NovoBolsaFamilia")
	}
	return nil
}
