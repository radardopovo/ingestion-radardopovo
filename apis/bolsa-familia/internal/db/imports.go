...
func GetImportState(ctx context.Context, db *sql.DB, period int) (*ImportState, error) {
	const q = `SELECT periodo_yyyymm, zip_sha256, status, last_step, rows_bolsa_familia, started_at, finished_at, error_msg
	FROM imports_bolsa_familia WHERE periodo_yyyymm = $1`
...
func UpsertImportState(ctx context.Context, db *sql.DB, st ImportState) error {
	const q = `
	INSERT INTO imports_bolsa_familia (
		periodo_yyyymm, zip_sha256, status, last_step,
		rows_bolsa_familia, started_at, finished_at, error_msg
	) VALUES (
		$1, $2, $3, $4,
		$5, $6, $7, $8
	)
	ON CONFLICT (periodo_yyyymm) DO UPDATE SET
		...
		rows_bolsa_familia = EXCLUDED.rows_bolsa_familia,
...
func PurgePeriod(ctx context.Context, db *sql.DB, period int) error {
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM bolsa_familia WHERE periodo_yyyymm = $1`, period); err != nil {
		return err
	}
	return tx.Commit()
}
