...
func (o *Orchestrator) ensureYearZIP(ctx context.Context, year int, dstPath string, st *db.ImportState) (string, error) {
	...
	url := fmt.Sprintf("https://portaldatransparencia.gov.br/download-de-dados/viagens/%d", year)
	...
	if string(head) != "PK\x03\x04" {
		return "", fmt.Errorf("resposta nao e um ZIP valido para o ano %d", year)
	}
