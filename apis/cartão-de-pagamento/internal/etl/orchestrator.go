...
import (
	...
	"github.com/radardopovo/cpgf-etl/internal/httpx"
	"github.com/radardopovo/cpgf-etl/internal/zipx"
)
...
func (o *Orchestrator) processPeriod(ctx context.Context, period int) (res PeriodStats, err error) {
	...
	if st != nil && st.Status == "done" && !o.cfg.Force && !o.cfg.OnlyDownload {
		if localSHA, ok := fileSHA256IfExists(zipPath); ok && localSHA == st.ZipSHA256 {
			o.log.Info("period_skipped_cache",
	...
	if o.cfg.OnlyDownload {
		o.log.Info("period_download_only_done",
			slog.Int("periodo", period),
	...
	stats, runErr := o.importCPGF(ctx, period, files.CPGF)
	if runErr != nil {
		o.markPeriodError(ctx, state, "cpgf", runErr)
		return res, runErr
	}

	state.RowsCPGF = stats.Inserted
...
func (o *Orchestrator) zipPath(period int) string {
	return filepath.Join(o.zipsDir(), fmt.Sprintf("cpgf_%d.zip", period))
}
